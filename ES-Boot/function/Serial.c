...
uint8_t ifConnected = 0;	  //记录是否已经连接
...
int i = 0; 
uint8_t count = 0;
uint8_t cipstate_count = 0;	//查询是否有TCP连接的计数
uint8_t State = 0;
uint8_t mode  = NONE;
...
void USART1_IRQHandler(void) 
{
	if (USART_GetITStatus(USART1, USART_IT_RXNE) == SET)		//判断是否是USART1的接收事件触发的中断
	{
		...
		if(!ifConnected && RxData == 0x0A && mode != CIPSTATE)
			count++;
		
		switch(mode){
			// 起始状态
			case NONE:
				if(count == 6){	//count为6说明成功连接wifi,跳转到WIFI模式
						clear();
						mode = WIFI;
				}else{
					break;
			...
			//获取数据模式
			case RCV_DATA:
				insertData(RxData, &socketPacket);
				break;
			//记录数据包个数
			case CIPSTATE:
				cipstate_count++;
...
uint8_t moveFstPos(struct buffer* Packet,uint8_t step){
	if(Packet->len < step){
		//OLED_ShowNum(3,1,Packet->len,4);
		//OLED_ShowNum(3,10,step,3);
		return 0;
	}
...
void SendPacketHeader(uint8_t type, uint32_t packet_len){
	...
	header[4] = type;
	header[5] = packet_len & 0x00FF0000;
	header[6] = packet_len & 0x0000FF00;
